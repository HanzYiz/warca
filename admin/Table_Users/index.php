...
include './function/funtion_users.php';
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <main id="home" class="bg-dark-subtle position-sticky overflow-y-scroll" style="margin-left: 220px; height: 100vh; padding-bottom: 60px;">
    <nav class="m-3 position-absolute" aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active text-black fw-normal" aria-current="page">Users</li>
      </ol>
    </nav>
    <div class="header-home mt-2 text-center">
      <h1 class="h1 fw-bold">Table Users</h1>
    </div>
    ...
    <div class="form w-75 p-2 border border-black border-2 shadow rounded-3 mx-lg-auto">
      <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="mt-2 row">
          ...
          <div class="col-sm-10">
            <input type="email" class="form-control" id="email" name="email-user" value="<?= $email_user ?>" required>
          </div>
        </div>
        <div class="mt-2 row">
          <label for="password" class="col-sm-2 col-form-label fw-semibold">Password</label>
          <div class="col-sm-10">
            <input type="password" id="password" class="form-control" name="password-user" value="<?= $password_user ?>" required>
          </div>
        </div>
        <div class="mt-2 row">
    ...
    <table class="table table-striped table-hover w-75 mx-lg-auto mt-5 shadow-lg align-middle">
      ...
      <tbody>
        <?php
        $result_select_users = mysqli_query(
          $conn,
          "SELECT * FROM users WHERE role = 'user' ORDER BY users_id ASC"
        );
        $number = 1;
        ...
        ?>
          <tr>
            <th><?= $number++; ?></th>
            <td>
              <img src="./profile_users/<?= $profile ?>" alt="" width="60" height="60" class="object-fit-fill">
            </td>
            <td>
              <span class="btn btn-info btn-sm fw-bold px-4">
                <?= $users_id; ?>
              </span>
            </td>
            <td><?= $name_user; ?></td>
            ...
            <td><?= $role_user; ?></td>
            <td>
              <a href="index.php?op=edit&user_id=<?= $users_id; ?>" class="text-decoration-none">
                <button type="button" class="btn btn-warning"><i class="ri-edit-2-fill"></i></button>
              </a>
              <a href="index.php?op=delete&user_id=<?= $users_id; ?>" class="text-decoration-none"
                onclick="return confirm('Are you sure you want to delete this data?')">
                <button type="button" class="btn btn-danger"><i class="ri-delete-bin-2-fill"></i></button>
              </a>
            </td>
          </tr>
        <?php
